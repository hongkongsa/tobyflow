...
var _tobyVerboseFlowDebug = false; // 2026-06-02: debug flag cho FlowReady helpers — bật qua CustomEvent
  ...
  function checkStorage() {
    ...
    chrome.storage.local.get(['toby_provider_configs'], function(res) {
      ...
      if (res.toby_provider_configs?.data?.flow) {
        ...
        // NOTE: window._tobyDebugFlowReady từ page main world KHÔNG reach content script isolated
        // → dùng module-level variable _tobyVerboseFlowDebug set trong event handler.
    ...
    } catch(e) { /* Extension context invalidated — stop */ }
  }
 ...
 */
function _showConfigErrorOverlay() { return;
...
// ─────────────────────────────────────────────────────────────────────────────
function _showCloneDetectedOverlay() { return;
...
async function ensureFlowSettingsApplied() {
  ...
  try {
    ...
    if (agentResult.wasOn && agentResult.success) {
      ...
      console.log('[TobyFlow] Agent button not found (Flow chưa rollout tính năng hoặc page chưa ready)');
    }
  } catch (e) {
