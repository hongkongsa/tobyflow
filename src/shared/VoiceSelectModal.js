...
class VoiceSelectModal {
        ...
        <div class="style-modal-content">
          <div class="style-modal-header">
            ...
            <div class="voice-modal-header-actions">
              ${VoiceSelectModal._isAdmin() ? `
                <button type="button" class="voice-modal-sync-btn" id="voiceModalSyncBtn" title="${t('settings.resyncVoices', 'Resync Voices')} (admin)">
  ...
  static _bindEvents() {
    ...
    // Sync voices button — CHỈ render + bind cho admin (xem _render() conditional)
    // Defensive double-check: skip binding nếu button không tồn tại (non-admin) hoặc role đổi giữa chừng.
    const syncBtn = overlay.querySelector('#voiceModalSyncBtn');
    if (syncBtn && VoiceSelectModal._isAdmin()) {
  ...
  static async _handleSyncClick() {
    ...
    try {
      ...
      if (btn) btn.title = `Sync failed: ${e.message || e}`;
    } finally {
