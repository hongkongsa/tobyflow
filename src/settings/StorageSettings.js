...
class StorageSettings {
  ...
  async init() {
    ...
    // [Dev Fix] Ensure ModelRegistry has data BEFORE _hydrateModelDefaults is called.
    // Race condition: _loadServerDefaults() completes quickly (local server) → _hydrateModelDefaults()
    // called while ModelRegistry.fetch() is still in-flight → cache_empty warning.
    // Await ModelRegistry.fetch() here to guarantee _cache is populated before hydrate.
    if (window.ModelRegistry) {
      await window.ModelRegistry.fetch().catch(() => {});
    }
    ...
    // Prevent race condition: wait for AuthManager to complete initialization
    // so currentUserId is populated correctly before loadAndApply calls _validateLocalOwner
    if (window.authManager && typeof window.authManager._initPromise !== 'undefined') {
      try { await window.authManager._initPromise; } catch (_) {}
    }
    ...
    window.eventBus?.on?.('provider:models_updated', () => {
      this._hydrateModelDefaults();
      // Also propagate updated defaults into current settings
      if (!this.settings.defaultImageModel && this.defaults.defaultImageModel) {
        this.settings.defaultImageModel = this.defaults.defaultImageModel;
      }
      if (!this.settings.defaultVideoModel && this.defaults.defaultVideoModel) {
        this.settings.defaultVideoModel = this.defaults.defaultVideoModel;
      }
      if (!this.settings.telegramFlowModel && this.defaults.telegramFlowModel) {
        this.settings.telegramFlowModel = this.defaults.telegramFlowModel;
      }
    });
  }
